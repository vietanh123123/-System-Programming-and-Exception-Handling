    ...
    */
    def apply(input: String): String = {
        ...
        try {
          ...
          case e: Exception => { throw new RISCVAssemblerException("Error in preprocessing la and li instructions.") }
        }
...
protected object InstructionParser {
  ...
  def parseImmediate(imm: String, input: String): Long = {
    try {
      ...
      case e: Exception => throw new RISCVAssemblerException(s"Invalid immediate value ${imm}. (in line: $input)")
    }
  }
      ...
      case InstType.I => {
          ...
          val imm = 
            inst.name match {
              case "LA2" => try {
                ...
                case e: Exception => throw new RISCVAssemblerException(s"Label ${instructionParts(2)} not found. (in line: $input)")
              }
        ...
        if (inst.hasOffset) {
          val imm = try { 
              ...
              case e: Exception => throw new RISCVAssemblerException(s"Invalid immediate value ${instructionParts(2)}. (in line: $input)")
            }
          Some(
            (
              inst,
...
protected object RegMap {
  ...
  def apply(input: String): Long =
    try {
      ...
      case e: Exception => throw new RISCVAssemblerException(s"Error in register mapping. Unknown register $input")
    }
}
...
protected object CsrMap {
  ...
  def apply(input: String): Long =
    try{
      ...
      case e: Exception => throw new RISCVAssemblerException(s"Error in CSR mapping. Unknown CSR $input")
    }
}